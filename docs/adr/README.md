...
| [0011](0011-daily-activity-reminder.md) | guildごとにopt-inで設定した時刻(JST)に、その日が活動あり(active / active-anytime)の日程調整をイベントごとに1通、guild設定のリマインドチャンネルへ自動投稿し、回答履歴のあるユーザーを参加者としてメンションする。croner による毎分tick + DBの判定済み記録で「設定時刻を過ぎて当日未判定なら判定・送信する」を実現し、判定は発火時の1回きりとする | accepted | architecture, db, feature, stack |
